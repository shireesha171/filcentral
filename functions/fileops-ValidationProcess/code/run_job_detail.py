...
def run_job_detail_fun(run_job_id, job_validation_status):
    ...
    try:
        ...
        job_query = """
          ...
          ,j.file_name ,j.file_size ,jr.start_time ,jr.end_time,jr.job_type, j.schedule_json, tc.connectivity_type ,bup.file_name as source_file_name,
          tc.absolute_file_path  as target_absolute_file_path,
          sc.absolute_file_path as source_absolute_file_path,
          jr.source_file_location
          from  job_runs as jr
        ...
        if records:
            ...
            if record is not None and record['target_id'] is not None:
                ...
                s3_file_target_location = "target-files/" + s3_key + "/" + record['job_uuid'] + "/" + "data.csv"
            ...
            if record is not None and record['source_file_location'] is not None:
            ...
            if record is not None and record['target_id'] is not None:
                ...
                target_data = downloads3(result[1],record['job_uuid'],'target')
...
def downloads3( file_path,job_uuid,type):
